...
#include "../logic/render.h"
#include "../types/base.h"
#include "../types/button_types.h"
...
SDL_FRect button_get_rect(struct button_t *button) {
  ...
  if (button->label.text == NULL ||
      !TTF_GetTextSize(button->label.text, &text_w, &text_h)) {
    return rect;
  }
  if (rect.h < 0) {
    if (text_h < 0) {
...
bool button_render(struct base_t *obj, struct render_ctx_t *ctx) {
  ...
  if (!SDL_SetRenderDrawColor(ren, t.color.r, t.color.g, t.color.b,
                              t.color.a)) {
    return false;
  }
  ...
  (void)label_set_center_pos(&b->label, rect.x + rect.w / 2,
                             rect.y + rect.h / 2);
...
struct render_t button_get_render(struct button_t *button) {
  ...
  result.get_viewable_rect = button_get_viewable_rect;
  return result;
}

void button_free(struct button_t *button) { label_free(&button->label); }
